...
def doProduce(file_path, file_type):
...
def process_file_and_store(file_path, file_type):
    ...
    try:
        for chunk_path in chunk_json_path_list:
            ...
            db.insert_into_aigcFileList(conn, dataDO)
            ...
            if not processing_result['success']:
                ...
                return
            # insert chunk path to READY
            dataDO = buildData(chunk_path, file_type, 'FINISH');

            # 将处理后的数据插入到aigcFileList表中
            db.updateStatus(conn, dataDO,'FINISH')
            logging.info(f"File processed and stored successfully with data: {processed_data}")
